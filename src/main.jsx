...
import {isOrderFinalized, isPaymentOverdue, paymentSummary} from './orders/payments'
...
function Quote({settings,inventory,setInventory,catalog,quotes,setQuotes,orders,setOrders,clients,setClients,models,setModels,designDraft,onDraftConsumed,onOrderCreated}){
  ...
  const order=()=>{
    ...
    const inventoryDeductions=Object.entries(calc.inventoryUsage).map(([id,amount])=>({id,name:inventory.find(item=>item.id===id)?.name||'Filamento',amount,unit:'g'}))
    const createdOrder={...q,id:uid(),quoteId:q.id,status:'pending',createdAt:new Date().toISOString(),dueDate:'',paymentDueDate:'',paymentPlan:'deposit',payments:[],inventoryDeductions}
...
function ServiceQuote({service,inventory,setInventory,catalog:productCatalog,settings,quotes,setQuotes,orders,setOrders,clients,setClients,onOrderCreated}){
  ...
  const calc=useMemo(()=>{
    ...
    const inventoryConsumption=material.inventoryId&&includeMaterial?calculateCricutConsumption({width:w,height:h,quantity:qty,piecesPerSheet:perSheet,trackingMode:material.trackingMode}):null
  ...
  const snapshot=()=>({id:uid(),folio:`COT-${Date.now().toString().slice(-7)}`,date:new Date().toISOString(),service:isLaser?'Láser':'Cricut',...form,materialName:form.materialSource==='studio'?material.name:'Sin material',catalogName:selectedProduct?.name||'',typeName,...calc})
  ...
  const order=()=>{
    ...
    const inventoryDeductions=[
      ...(productItem?[{id:productItem.id,name:productItem.name,amount:num(form.quantity),unit:'pza'}]:[]),
      ...(materialItem&&calc.inventoryConsumption?[{id:materialItem.id,name:materialItem.name,amount:calc.inventoryConsumption.amount,unit:calc.inventoryConsumption.unit}]:[]),
    ]
    const createdOrder={...q,id:uid(),quoteId:q.id,status:'pending',createdAt:new Date().toISOString(),dueDate:'',paymentDueDate:'',paymentPlan:'deposit',payments:[],inventoryDeductions}
    setOrders([createdOrder,...orders]);onOrderCreated?.(createdOrder)
  ...
  return <div className="quoteLayout"><Card title={`Cotizador ${isLaser?'láser':'Cricut'}`} subtitle="Medidas y tiempo de máquina para un resultado realista.">
    <Field label="Producto base del inventario o catálogo (opcional)" full><select value={form.catalogProduct} onChange={e=>{const item=catalogProducts.find(p=>p.id===e.target.value)||inventoryProducts.find(p=>`stock-${p.id}`===e.target.value);setForm({...form,catalogProduct:e.target.value,project:form.project||item?.name||'',materialSource:isLaser&&item?'catalog':'studio'});setSaved(false)}}><option value="">Cotización desde cero</option>{inventoryProducts.length>0&&<optgroup label="Inventario físico — descuenta al crear pedido">{inventoryProducts.map(p=><option key={p.id} value={`stock-${p.id}`}>{p.name} — {money(num(p.salePrice)||unitCost(p))} · {num(p.stock)} disponibles</option>)}</optgroup>}{catalogProducts.length>0&&<optgroup label="Catálogo — no descuenta inventario">{catalogProducts.map(p=><option key={p.id} value={p.id}>{p.name} — {money(p.price)}</option>)}</optgroup>}</select></Field>
    ...
    <Field label="Origen del material o pieza"><select value={form.materialSource} onChange={e=>update('materialSource',e.target.value)}><option value="studio">Lo proporciona A&E</option><option value="none">Sin material / ya incluido</option><option value="client">Lo proporciona el cliente</option>{selectedProduct&&isLaser&&<option value="catalog">Incluido en el precio del producto</option>}</select></Field>
    <Field label="Material"><select value={form.materialSource==='studio'?form.material:'__none__'} onChange={e=>{const value=e.target.value;setForm({...form,materialSource:value==='__none__'?'none':'studio',material:value==='__none__'?form.material:value});setSaved(false)}}><option value="__none__">Sin material (no sumar costo)</option><optgroup label="Materiales configurados">{baseCatalog.map(m=><option key={m.id} value={m.id}>{m.name} — {money(m.sheetCost)}</option>)}</optgroup>{inventoryCatalog.length>0&&<optgroup label={`Inventario de ${isLaser?'Láser':'Cricut'}`}>{inventoryCatalog.map(m=><option key={m.id} value={m.id}>{m.name} — {money(m.sheetCost)} por compra</option>)}</optgroup>}</select></Field>
    ...
    {selectedProduct&&<div className="usageNote catalogQuoteNote"><b>{selectedProduct.name}: {money(selectedProduct.price)} por pieza</b><span>{selectedProduct.source==='inventory'?`Inventario físico: ${num(inventoryProduct?.stock)} disponibles; al crear este pedido quedarán ${Math.max(0,num(inventoryProduct?.stock)-num(form.quantity))}.`:calc.addProductToService?'El producto base se suma al material y al trabajo de personalización. No descuenta inventario porque procede del Catálogo.':'Este precio se respeta como mínimo. No descuenta inventario porque procede del Catálogo.'}</span></div>}
    <div className="usageNote">{form.materialSource==='catalog'?<><b>Producto incluido en el precio seleccionado</b><span>No se suma nuevamente como material. Si necesitas recalcularlo con un costo de compra actual, cambia el origen a “Lo proporciona A&E”.</span></>:form.materialSource==='none'?<><b>Sin material adicional</b><span>El costo del material es $0 porque el precio elegido ya lo incluye o este trabajo no lo requiere.</span></>:form.materialSource==='client'?<><b>Material proporcionado por el cliente</b><span>El costo del material es $0. Solo se cobran máquina, diseño, mano de obra, acabado y extras.</span></>:calc.isUnit?<><b>{form.quantity} producto(s) a {money(material.sheetCost)} cada uno</b><span>Costo del producto base: {money(calc.materialCost)}. Configura el costo de compra por pieza en Inventario.</span></>:calc.isArea?<><b>{calc.usedArea.toFixed(2)} cm² de material para este trabajo</b><span>Costo proporcional: {money(calc.materialCost)}. Al crear el pedido se descontarán {calc.inventoryConsumption?.amount.toFixed(2)} cm² del inventario.</span></>:calc.isMeter?<><b>{calc.across} pieza(s) a lo ancho · {calc.usedLength.toFixed(1)} cm lineales utilizados</b><span>Costo del vinil utilizado: {money(calc.materialCost)} · rollo de {material.sheetWidth} cm de ancho a {money(material.sheetCost)} por metro · merma {material.waste||0}%.</span></>:<><b>{calc.perSheet} pieza(s) por placa/hoja · {calc.sheets} necesarias</b><span>Costo proporcional por pieza: {money(calc.materialUnit)} · formato de {material.sheetWidth} × {material.sheetHeight} cm a {money(material.sheetCost)} · merma {material.waste||0}%.{calc.inventoryConsumption?` Al crear el pedido se descontarán ${calc.inventoryConsumption.amount} hoja(s).`:''}</span></>}</div>
...
function Orders({orders,setOrders}){
  ...
  const finalizedOrders=orders.filter(isOrderFinalized)
...
function ParametersPage(){
  const [service,setService]=useState('3d')
  const [selectedIds,setSelectedIds]=useState({
    '3d':productionProfiles['3d'][0].id,
    laser:productionProfiles.laser[0].id,
    cricut:productionProfiles.cricut[0].id,
  })
  const profiles=productionProfiles[service]||productionProfiles['3d']
  const selected=profiles.find(profile=>profile.id===selectedIds[service])||profiles[0]
  const values=selected.parameters
  const rows=service==='3d'?
    [['Boquilla',`${values.nozzle} mm`],['Altura de capa',`${values.layerHeight} mm`],['Paredes',values.walls],['Capas superiores e inferiores',values.topBottom],['Relleno',`${values.infill}%`],['Patrón de relleno',values.pattern],['Soportes',values.supports],['Adherencia',values.adhesion]]:
    service==='laser'?
      [['Velocidad',`${values.speed} mm/min`],['Potencia',`${values.power}%`],['Pasadas',values.passes],['Air Assist',values.airAssist],['Modo de potencia',values.mode],['Enfoque',values.focus]]:
      [['Ajuste en Design Space',values.designSpaceSetting],['Presión',values.pressure],['Pasadas',values.passes],['Cuchilla',values.blade],['Espejo',values.mirror],['Tapete',values.mat]]
  const title=service==='3d'?'Impresión 3D · Bambu Lab A1':service==='laser'?'Láser · Sculpfun 20 W':'Cricut'
  const chooseProfile=id=>setSelectedIds(current=>({...current,[service]:id}))
  return <>
    <section className="parameterLibraryHero"><div><span>Guía de producción</span><h2>Parámetros recomendados</h2><p>Consulta un punto de partida sin agregar más campos a tus cotizaciones.</p></div><BookOpen size={34}/></section>
    <div className="parameterServiceTabs" aria-label="Selecciona la máquina">
      <button className={service==='3d'?'active':''} onClick={()=>setService('3d')}><Box size={20}/><span><b>Impresión 3D</b><small>Bambu Lab A1</small></span></button>
      <button className={service==='laser'?'active':''} onClick={()=>setService('laser')}><Flame size={20}/><span><b>Láser</b><small>Sculpfun 20 W</small></span></button>
      <button className={service==='cricut'?'active':''} onClick={()=>setService('cricut')}><Scissors size={20}/><span><b>Cricut</b><small>Corte y papelería</small></span></button>
    </div>
    <Card title={title} subtitle="Elige el tipo de trabajo para consultar sus valores sugeridos.">
      <div className="parameterLookupHeader"><label><span>Tipo de trabajo</span><select aria-label="Tipo de trabajo para consultar" value={selected.id} onChange={event=>chooseProfile(event.target.value)}>{profiles.map(profile=><option key={profile.id} value={profile.id}>{profile.name}</option>)}</select></label><p>{selected.description}</p></div>
      <div className="parameterReferenceGrid">{rows.map(([label,value])=><article key={label}><span>{label}</span><b>{value}</b></article>)}</div>
      {service==='laser'&&<div className="parameterSafety"><b>Seguridad</b><p>Haz una prueba en un sobrante, mantén extracción y vigilancia. Nunca cortes vinil/PVC ni materiales de composición desconocida.</p></div>}
      {service==='cricut'&&<div className="parameterAdvice"><b>Antes del pedido</b><p>Realiza un corte de prueba. El desgaste de la cuchilla, la marca y el grosor del material pueden requerir ajustar la presión.</p></div>}
      {service==='3d'&&<div className="parameterAdvice"><b>Antes de imprimir</b><p>Revisa la orientación y la vista previa en Bambu Studio. El material y la geometría de la pieza pueden requerir soportes o adherencia adicional.</p></div>}
      <p className="parameterLibraryFootnote">Estos valores son una referencia inicial para producción. No cambian los costos ni el resultado de los cotizadores.</p>
    </Card>
  </>
}
