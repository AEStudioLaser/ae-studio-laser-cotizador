...
function ServiceQuote({service,inventory,setInventory,catalog:productCatalog,settings,quotes,setQuotes,orders,setOrders,clients,setClients}){
  ...
  const calc=useMemo(()=>{
    ...
    return{perSheet,sheets,materialUnit,materialCost,machineHours,machineCost,productionCost,catalogBase:productBase,productBase,addProductToService,total:pricing.total,unit:pricing.total/qty,profitAmount:pricing.profitAmount,personalizationPrice:pricing.personalizationPrice,isMeter,isUnit,isArea,usedArea,usedLength,across,inventoryConsumption}
  },[form,material,selectedProduct,isLaser,settings])
...
function Inventory({items,setItems}){
  ...
  const cricutRaw=form.type==='raw'&&form.category==='cricut',tracksArea=cricutRaw&&form.trackingMode==='area',tracksSheets=cricutRaw&&form.trackingMode==='sheets'
  ...
  return <><div className="stats compact"><Stat label="Artículos" value={items.length} tone="blue"/><Stat label="Dinero invertido" value={money(investment)} tone="purple"/><Stat label="Venta potencial" value={money(potential)} tone="green"/><Stat label="Stock bajo" value={items.filter(i=>available(i)<=num(i.minStock)).length} tone="orange"/></div>
    <Card title={editing?'Editar artículo':'Agregar al inventario'} subtitle="Registra productos, vinil por superficie o papel y stickers por hojas.">
      <div className="formGrid">
        <Field label="Tipo"><select value={form.type} onChange={e=>{const type=e.target.value;setForm({...form,type,trackingMode:type==='raw'&&form.category==='cricut'?'area':'units'})}}><option value="product">Producto para venta</option><option value="raw">Materia prima</option></select></Field>
        ...
        <Field label="Categoría del cotizador"><select value={form.category} onChange={e=>{const category=e.target.value;setForm({...form,category,trackingMode:form.type==='raw'&&category==='cricut'?'area':'units'})}}><option value="laser">Láser</option><option value="3d">Impresión 3D</option><option value="cricut">Cricut</option></select></Field>
        ...
        <Field label={tracksSheets?'Hojas compradas':tracksArea?'Rollos o tramos comprados':'Cantidad comprada'}><input type="number" min="1" value={form.purchaseQty} onChange={e=>setForm({...form,purchaseQty:e.target.value})}/></Field>
      ...
      <div className="calculated">{tracksArea?<><span>Superficie disponible <b>{displayArea.toFixed(2)} cm²</b></span><span>Equivale a <b>{remainingAreaLength(displayArea,form.materialWidth).toFixed(2)} cm lineales</b></span><span>Costo por cm² <b>{money(unitCost(form)/Math.max(1,num(form.materialWidth)*num(form.materialLength)))}</b></span></>:tracksSheets?<span>Costo por hoja <b>{money(unitCost(form))}</b></span>:<><span>Costo unitario <b>{money(unitCost(form))}</b></span>{form.type==='product'&&<span>Ganancia por unidad <b>{money(num(form.salePrice)-unitCost(form))}</b></span>}</>}</div>
    ...
    <Card title="Existencias" subtitle={`${shown.length} artículos`}><SearchBox value={search} setValue={setSearch} placeholder="Buscar material, producto o proveedor…"/>{shown.length?<div className="inventoryGrid">{shown.map(i=>{const isArea=i.trackingMode==='area',isSheets=i.trackingMode==='sheets',existence=isArea?`${num(i.areaRemaining).toFixed(2)} cm² (~${remainingAreaLength(i.areaRemaining,i.materialWidth).toFixed(2)} cm lineales)`:isSheets?`${num(i.stock)} hojas`:num(i.stock);return <article className="inventoryCard" key={i.id}><div className="inventoryIcon">{i.type==='raw'?<Box/>:<Package/>}</div><div className="grow"><div className="itemTitle"><h3>{i.name}</h3>{available(i)<=num(i.minStock)&&<span className="stockAlert">Stock bajo</span>}</div><p>{categoryName(i.category)} · {i.supplier||'Sin proveedor'}</p><div className="itemNumbers"><span>Existencia <b>{existence}</b></span><span>Costo {isArea?'por compra':isSheets?'por hoja':''}<b>{money(unitCost(i))}</b></span>{i.type==='product'&&<span>Venta <b>{money(i.salePrice)}</b></span>}</div></div><div className="rowActions"><button onClick={()=>{const category=['laser','3d','cricut'].includes(i.category)?i.category:'laser',trackingMode=i.type==='raw'&&category==='cricut'&&!['area','sheets'].includes(i.trackingMode)?'area':i.trackingMode||'units';setEditing(i.id);setForm({...freshStock,...i,category,trackingMode});scrollTo({top:0,behavior:'smooth'})}}>Editar</button><button className="iconButton danger" onClick={()=>confirm('¿Eliminar este artículo?')&&setItems(items.filter(x=>x.id!==i.id))}><Trash2 size={16}/></button></div></article>})}</div>:<Empty text="Todavía no hay artículos registrados."/>}</Card></>
}
