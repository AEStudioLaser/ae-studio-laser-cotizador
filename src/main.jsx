...
const freshQuote=s=>({client:'',project:'',catalogProduct:'',material:s.materials[0]?.id||'pla',quantity:1,weight:50,hours:4,minutes:0,extras:0,labor:0,failure:num(s.failureRate??10),profit:num(s.defaultProfit)||50})
const freshStock={type:'product',name:'',category:'laser',supplier:'',purchaseQty:1,purchaseTotal:0,stock:1,minStock:0,salePrice:0,purchaseDate:today()}
...
function Quote({settings,inventory,catalog,quotes,setQuotes,orders,setOrders,clients,setClients,models,setModels,designDraft,onDraftConsumed}){
  ...
  </Card><aside className="resultCard printArea"><img src="/logo-ae.png" className="printLogo" alt="A&E Studio Laser"/><span>Total sugerido</span><strong>{money(calc.total)}</strong><div className="unitPrice"><span>Precio por pieza</span><b>{money(calc.unit)}</b></div><h3>Desglose</h3>{calc.catalogBase>0&&<Line label="Precio base de catálogo" value={calc.catalogBase}/>}<Line label="Material" value={calc.materialCost}/><Line label="Electricidad" value={calc.electricity}/><Line label="Desgaste" value={calc.wear}/><Line label={`Reserva por fallas (${num(form.failure)}%)`} value={calc.failureCost}/><Line label="Mano de obra" value={form.labor}/><Line label="Extras" value={form.extras}/><Line label="Costos conocidos" value={calc.productionCost} bold/><Line label={catalogItem?'Diferencia sobre costos conocidos':'Ganancia'} value={calc.profitAmount}/><small>La reserva por fallas cubre material, electricidad y desgaste que habría que repetir. {catalogItem?'Se respeta como mínimo el precio vigente del catálogo. ':''}Vigencia: {settings.quoteValidity||15} días</small></aside></div>
}

function ServiceQuote({service,inventory,catalog:productCatalog,settings,quotes,setQuotes,orders,setOrders,clients,setClients}){
  const isLaser=service==='laser',baseCatalog=(isLaser?settings.laserMaterials:settings.cricutMaterials)|| (isLaser?defaults.laserMaterials:defaults.cricutMaterials),inventoryCatalog=(inventory||[]).filter(i=>i.category===service&&num(i.stock)>0).map(i=>({id:`stock-${i.id}`,name:`${i.name} (Inventario: ${num(i.stock)})`,pricingMode:'unit',sheetWidth:0,sheetHeight:0,sheetCost:unitCost(i),waste:0,inventoryId:i.id})),catalog=[...baseCatalog,...inventoryCatalog],catalogProducts=(productCatalog||[]).filter(p=>(p.service===service||p.service==='all')&&p.status!=='hidden')
  ...
  const update=(k,v)=>{setSaved(false);setForm({...form,[k]:v})},material=catalog.find(m=>m.id===form.material)||catalog[0]||{name:'Material',sheetWidth:1,sheetHeight:1,sheetCost:0,waste:10},catalogItem=catalogProducts.find(p=>p.id===form.catalogProduct)
  const calc=useMemo(()=>{
    const w=Math.max(.1,num(form.width)),h=Math.max(.1,num(form.height)),qty=Math.max(1,num(form.quantity)),sw=Math.max(.1,num(material.sheetWidth)),sh=Math.max(.1,num(material.sheetHeight)),waste=Math.max(0,num(material.waste))/100,isMeter=material.pricingMode==='meter',isUnit=material.pricingMode==='unit'
    ...
    const proportionalUnit=num(material.sheetCost)/perSheet,includeMaterial=form.materialSource==='studio',automaticMaterial=isUnit?num(material.sheetCost)*qty:isMeter?(usedLength/100)*num(material.sheetCost):(form.fullSheet?sheets*num(material.sheetCost):proportionalUnit*qty),materialCost=includeMaterial?(form.manualMaterial===''?automaticMaterial:num(form.manualMaterial)):0,materialUnit=materialCost/qty,machineHours=num(form.hours)+num(form.minutes)/60,rate=isLaser?num(settings.laserRate||defaults.laserRate):num(settings.cricutRate||defaults.cricutRate),machineCost=machineHours*rate,productionCost=materialCost+machineCost+num(form.labor)+num(form.design)+num(form.assembly)+num(form.extras),raw=productionCost*(1+num(form.profit)/100),catalogBase=num(catalogItem?.price)*qty,step=Math.max(1,num(settings.roundTo)),total=Math.ceil(Math.max(raw,catalogBase)/step)*step
    return{perSheet,sheets,materialUnit,materialCost,machineHours,machineCost,productionCost,catalogBase,total,unit:total/qty,profitAmount:total-productionCost,isMeter,isUnit,usedLength,across}
  },[form,material,catalogItem,isLaser,settings])
  ...
  const snapshot=()=>({id:uid(),folio:`COT-${Date.now().toString().slice(-7)}`,date:new Date().toISOString(),service:isLaser?'Láser':'Cricut',...form,materialName:material.name,catalogName:catalogItem?.name||'',typeName,...calc})
  ...
  return <div className="quoteLayout"><Card title={`Cotizador ${isLaser?'láser':'Cricut'}`} subtitle="Medidas y tiempo de máquina para un resultado realista.">
    <Field label="Producto del catálogo (opcional)" full><select value={form.catalogProduct} onChange={e=>{const item=catalogProducts.find(p=>p.id===e.target.value);setForm({...form,catalogProduct:e.target.value,project:form.project||item?.name||'',materialSource:item?'catalog':'studio'});setSaved(false)}}><option value="">Cotización desde cero</option>{catalogProducts.map(p=><option key={p.id} value={p.id}>{p.name} — {money(p.price)}</option>)}</select></Field>
    ...
    <Field label="Origen del material o pieza"><select value={form.materialSource} onChange={e=>update('materialSource',e.target.value)}><option value="studio">Lo proporciona A&E</option><option value="client">Lo proporciona el cliente</option>{catalogItem&&<option value="catalog">Incluido en el precio del catálogo</option>}</select></Field>
    <Field label="Material"><select disabled={form.materialSource!=='studio'} value={form.material} onChange={e=>update('material',e.target.value)}><optgroup label="Materiales configurados">{baseCatalog.map(m=><option key={m.id} value={m.id}>{m.name} — {money(m.sheetCost)}</option>)}</optgroup>{inventoryCatalog.length>0&&<optgroup label={`Inventario de ${isLaser?'Láser':'Cricut'}`}>{inventoryCatalog.map(m=><option key={m.id} value={m.id}>{m.name} — {money(m.sheetCost)} c/u</option>)}</optgroup>}</select></Field>
    ...
    {form.advanced&&<div className="formGrid advancedBox"><Field label="Diseño / preparación ($)"><input type="number" min="0" value={form.design} onChange={e=>update('design',e.target.value)}/></Field><Field label="Armado / acabado ($)"><input type="number" min="0" value={form.assembly} onChange={e=>update('assembly',e.target.value)}/></Field><Field label="Extras ($)"><input type="number" min="0" value={form.extras} onChange={e=>update('extras',e.target.value)}/></Field><Field label="Costo de material manual ($)"><input type="number" min="0" value={form.manualMaterial} placeholder={money(calc.materialCost)} onChange={e=>update('manualMaterial',e.target.value)}/></Field>{!calc.isMeter&&!calc.isUnit&&<label className="checkField"><input type="checkbox" checked={form.fullSheet} onChange={e=>update('fullSheet',e.target.checked)}/><span>Cobrar placa u hoja completa</span></label>}</div>}
    {catalogItem&&<div className="usageNote catalogQuoteNote"><b>{catalogItem.name}: {money(catalogItem.price)} por pieza</b><span>Este precio de catálogo se respeta como mínimo. No descuenta existencias porque el producto está marcado como sobre pedido.</span></div>}
    <div className="usageNote">{form.materialSource==='catalog'?<><b>Producto incluido en el precio del catálogo</b><span>No se suma nuevamente como material. Si necesitas recalcularlo con un costo de compra actual, cambia el origen a “Lo proporciona A&E”.</span></>:form.materialSource==='client'?<><b>Material proporcionado por el cliente</b><span>El costo del material es $0. Solo se cobran máquina, diseño, mano de obra, acabado y extras.</span></>:calc.isUnit?<><b>{form.quantity} producto(s) a {money(material.sheetCost)} cada uno</b><span>Costo del producto base: {money(calc.materialCost)}. Configura el costo de compra por pieza en Configuración.</span></>:calc.isMeter?<><b>{calc.across} pieza(s) a lo ancho · {calc.usedLength.toFixed(1)} cm lineales utilizados</b><span>Costo del vinil utilizado: {money(calc.materialCost)} · rollo de {material.sheetWidth} cm de ancho a {money(material.sheetCost)} por metro · merma {material.waste||0}%.</span></>:<><b>{calc.perSheet} pieza(s) por placa/hoja · {calc.sheets} necesarias</b><span>Costo proporcional por pieza: {money(calc.materialUnit)} · formato de {material.sheetWidth} × {material.sheetHeight} cm a {money(material.sheetCost)} · merma {material.waste||0}%.</span></>}</div>
  ...
  </Card><aside className="resultCard printArea"><img src="/logo-ae.png" className="printLogo" alt="A&E Studio Laser"/><span>Total sugerido</span><strong>{money(calc.total)}</strong><div className="unitPrice"><span>Precio por pieza</span><b>{money(calc.unit)}</b></div><h3>Desglose</h3>{calc.catalogBase>0&&<Line label="Precio base de catálogo" value={calc.catalogBase}/>}<Line label="Material" value={calc.materialCost}/><Line label="Tiempo de máquina" value={calc.machineCost}/><Line label="Diseño" value={form.design}/><Line label="Mano de obra" value={form.labor}/><Line label="Armado / acabado" value={form.assembly}/><Line label="Extras" value={form.extras}/><Line label="Costos conocidos" value={calc.productionCost} bold/><Line label={catalogItem?'Diferencia sobre costos conocidos':'Ganancia'} value={calc.profitAmount}/><small>{catalogItem?'Precio de catálogo usado como mínimo · ':''}{typeName} · {form.width} × {form.height} cm</small></aside></div>
}
